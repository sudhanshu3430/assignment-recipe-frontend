...
const SignUpForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://assignment-recipe-backend.onrender.com/api/signup', { username, firstName, lastName, password });
      ...
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
              ...
              <input type="text" id="firstName" value={firstName} onChange={(e) => setFirstName(e.target.value)} required className="border border-gray-300 rounded-md px-3 py-2 w-full mt-1" />
            </div>
            <div className="mb-4">
              <label htmlFor="lastName" className="block text-gray-700 font-semibold">Last Name:</label>
              <input type="text" id="lastName" value={lastName} onChange={(e) => setLastName(e.target.value)} required className="border border-gray-300 rounded-md px-3 py-2 w-full mt-1" />
            </div>
          <div className="mb-4">
          ...
          <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full">Sign Up</button>
